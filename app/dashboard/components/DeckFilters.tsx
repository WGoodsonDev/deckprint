...
export function DeckFilters({ availableFormats, filters, onChange }: DeckFiltersProps) {
  ...
  return (
    <div className="mb-3 flex flex-wrap items-end gap-3 rounded-lg border border-zinc-200 bg-white p-3">
      <div className="flex flex-col gap-1">
        <label htmlFor="deck-filter-format" className="text-xs text-zinc-500">
          Format
        </label>
        <select
            ...
            return (
              <button
                ...
                className={`h-6 w-6 rounded-full ring-1 transition-opacity ${
                  active ? 'ring-2 ring-indigo-400' : 'ring-zinc-300 opacity-40 hover:opacity-70'
                }`}
      ...
      <div className="flex flex-col gap-1">
        <span className="text-xs text-zinc-500">Card count</span>
        <div className="flex items-center gap-1">
          <input
            ...
            className="w-16 rounded-md border border-zinc-200 bg-white px-2 py-1 text-sm text-zinc-700"
          />
